...
class TaskDecomposer:
    ...
    def _software(self, intent: Intent) -> List[TaskDraft]:
        return [
            ...
            TaskDraft("implementation", "developer", "实现功能并运行本地检查", depends_on=["architecture", "tests"], expected_outputs=["src/"], required_tools=["file_editor", "shell", "git"]),
            TaskDraft("review", "reviewer", "执行测试、构建和代码审查", depends_on=["implementation"], expected_outputs=["test-report.json"], required_tools=["shell", "test_runner"]),
        ]
...
class Planner:
    ...
    def build(self, intent: Intent, drafts: Sequence[TaskDraft]) -> Plan:
        ...
        final_checks = [check.id for task in drafts for check in task.checks if task.id in ("review", "experiment", "implementation")]
